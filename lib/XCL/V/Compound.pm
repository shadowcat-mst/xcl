...
async sub evaluate_against ($self, $scope) {
  ...
  my $res = await concat $scope->eval($val);
  return $res unless $res->is_ok;
  foreach my $step (@rest) {
    $res = await concat $res->val->invoke(
  ...
  while (my $step = shift @rest) {
    ...
    unless (@rest) {
      return $_ for not_ok_except NO_SUCH_VALUE =>
        my $lres = await concat dot_lookup_escape(
          $scope, $res->val, 'assign_via_call'
        );
      if ($lres->is_ok) {
        return await concat $lres->val->invoke(
          $scope, List[$step_list, $lst->values]
        );
      }
    }
    return $_ for not_ok $res = await concat $res->val->invoke($scope, $step_list);
  }
  return await concat dot_call_escape($scope, $res->val, assign => $lst->values);
}
